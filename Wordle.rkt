...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |hw9-problem1 2|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define GAP (square 32 "solid" BG-COLOR))
...
(check-expect
 (boxed-letter "A" 100 (λ (_) empty-image))
 (text "A" 50 BG-COLOR))

(check-expect
 (boxed-letter "B" 100 (λ (s) (overlay (square s "outline" BORDER-COLOR)
                                       (square s "solid" GUESS-COLOR))))
 (overlay
  (text "B" 50 BG-COLOR)
  (square 100 "outline" BORDER-COLOR)
  (square 100 "solid" GUESS-COLOR)))
...
(define (value-in-list? x lox func)
  (ormap (λ (y) (func y x)) lox))
...
(define (read-dictionary fpath)
  (local [
          (define FILE (string-append "/Users/krishbansal/Desktop/College/file for hw9/" fpath))]
    (if
     (file-exists? FILE )
...
(define (score guessed correct)
  (local [
          (define EXPLODED-GUESS-LIST (explode guessed))
          (define CORRECT-GUESS-LIST (explode correct))

          ;func-that-creates-new-pair : 1String 1String -> Pair
          ; determines if the guessed letter in the list is correct, just misplaced, or wrong
          ; by comparing it to the letters in the correct list

          (define (func-that-creates-new-pair l1 l2)
            (cond
              [(string=? l1 l2)
               (make-pair l1 "right")]
              [(value-in-list? l1 CORRECT-GUESS-LIST string=?)
               (make-pair l1 "misplaced")]
              [else
               (make-pair l1 "wrong")]))]
    (mymap2 EXPLODED-GUESS-LIST CORRECT-GUESS-LIST func-that-creates-new-pair)))
...
(define (partial-guess->full-guess s len)
  (implode (append (explode s) (build-list (- len (string-length s)) 
                                           (λ (var) " ")))))       
...
(define NUMBERS (list "1" "2" "3" "4" "5" "6" "7" "8" "9" "0"))
...
(define (key-wordle ws key low wlen)
  (local [; key-wordle/backspace: WordleState -> WordleState
          ; Returns a new WordleState when a backspace key is pressed making
          ; relevant modification to the current WordleState

          (define (key-wordle/backspace ws)
            (make-wordle
             (wordle-past ws)
             (remove-let (wordle-current ws))))
          
          ; remove-let: String -> String
          ; Returns a new String with one less letter
          
          (define (remove-let str)
            (if (> (string-length str) 0)
                (substring str 0 (sub1 (string-length str)))
                str))
          
          ; key-wordle/return: WordleState -> WordleState
          ; Returns a new WordleState when a return key is pressed making
          ; relevant modification to the current WordleState
          
          (define (key-wordle/return ws)
            (cond
              [(string-in-list? (wordle-current ws) low)
               (make-wordle (cons (wordle-current ws) (wordle-past ws)) "")]
              [else ws]))

          ; key-wordle/other: WordleState -> WordleState
          ; Returns a new WordleState when any other key is pressed making
          ; relevant modification to the current WordleState
          
          (define (key-wordle/other ws key)
            (if (and (string-alphabetic? key) (not (= (string-length (wordle-current ws)) wlen)))
                (make-wordle (wordle-past ws) (string-append (wordle-current ws) key))
                ws))]
    
    (cond
      [(key=? key "\r") (key-wordle/return ws)]
      [(key=? key "\b") (key-wordle/backspace ws)]
      [else (key-wordle/other ws (string-upcase key))])))
...
(define (end-wordle? ws cword maxguess)  
  (cond
    [(string-in-list? cword (wordle-past ws)) #t]
    [(= (length (wordle-past ws)) maxguess) #t]
    [else #f]))
...
(define (draw-end-wordle ws cword maxguess)
  (local [
          (define WIN-SCREEN (overlay (text (string-append
                                             "YOU WON!\n GUESSES: "
                                             (number->string (length (wordle-past ws))))
                                            20 "white")
                                      (rectangle 200 200 "solid" "dark green")))
          
          (define LOST-SCREEN (overlay (text "YOU LOST :(" 20 "white")
                                       (rectangle 200 200 "solid" "red")))]
          
    (cond
      [(string-in-list? cword (wordle-past ws)) WIN-SCREEN]
      [(= (length (wordle-past ws)) maxguess) LOST-SCREEN])))
...
(define GAMESCREEN (overlay (text "WORDLE" 50 "white")(rectangle 600 100 "solid" "black")))

(define (draw-wordle ws cword maxguess)
  (local [
          (define (func-x str)
            (row->image (map (λ (x) (make-pair x lsp->image)) (score str cword))))

          (define (past-guess-list ws)
            (reverse (map func-x (wordle-past ws))))

          (define (current-guess-image ws)
            (list (partial-guess->image (wordle-current ws) (string-length cword))))

          (define (future-guess-list ws)
            (map (λ (x) (partial-guess->image "" (string-length cword)))
                 (build-list
                  (- (- maxguess 1) (length (wordle-past ws))) add1)))]
    
    (above GAMESCREEN
           (stack/v (append (past-guess-list ws)
                            (current-guess-image ws)
                            (future-guess-list ws))))))
